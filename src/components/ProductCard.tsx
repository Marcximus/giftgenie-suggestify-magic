...
export const createDisplayTitle = (title: string): string => {
  ...
  // Look for the first meaningful product description
  const parts = cleanTitle.split(/[,|\-\(\[]/);
  let mainTitle = parts[0].trim();

  // If the main title is too short, try to include more meaningful information
  if (mainTitle.length < 15 && parts.length > 1) {
    mainTitle = parts.slice(0, 2).join(' - ').trim();
  }

  // Capitalize first letter of each word, except for common words
  const commonWords = ['a', 'an', 'the', 'and', 'but', 'or', 'for', 'nor', 'on', 'at', 'to', 'of', 'in', 'with'];
  const properCase = mainTitle.toLowerCase().split(' ').map((word, index) => {
    // Keep acronyms and product codes uppercase
    if (word.match(/^[A-Z0-9]+$/i) && word.length <= 5) {
      return word.toUpperCase();
    }
    // Don't lowercase common words unless they're the first word
...
const ProductCardComponent = ({ 
  ...
  const displayTitle = createDisplayTitle(title);
