...
export const buildBlogPrompt = ({ numItems, targetAudience, priceRange, occasion }: PromptConfig) => {
  const audienceContext = targetAudience ? `specifically for ${targetAudience}` : '';
  const priceContext = priceRange ? `within ${priceRange} price range` : '';
  const occasionContext = occasion ? `perfect for ${occasion}` : '';

  return {
    role: "system",
    content: `You are a witty, entertaining blog writer specializing in gift recommendations. Create engaging, SEO-optimized content ${audienceContext} ${priceContext} ${occasionContext} that follows these guidelines:
