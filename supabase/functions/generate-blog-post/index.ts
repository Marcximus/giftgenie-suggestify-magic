...
serve(async (req) => {
  ...
  try {
    const { title, numItems = 8 } = await req.json();
    console.log('Processing blog post for title:', title, 'with', numItems, 'items');

    // Get the prompt from promptBuilder with the specified number of items
    ...
    const response = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          {
            role: "user",
            content: `Create a fun, engaging blog post about: ${title}\n\nIMPORTANT: Generate exactly ${numItems} product suggestions.`
          }
        ],
        temperature: 0.7,
        max_tokens: 3500,
